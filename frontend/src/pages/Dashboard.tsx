    ...
    <div className="space-y-6 text-gray-900 dark:text-gray-200">
      ...
      <section className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm rounded-xl p-5 space-y-4">
        ...
        <form onSubmit={handleUpload} className="flex flex-wrap gap-3 items-center">
          <input type="file" onChange={handleFileChange} className="text-sm bg-white dark:bg-gray-900 p-1 rounded" />
          <button
            type="submit"
            disabled={uploading}
      ...
      {/* INFORME GENERAL */}
      <section className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm rounded-xl p-5 space-y-4">
        <h2 className="font-semibold text-lg text-gray-800 dark:text-gray-100">Exportar informe general</h2>

        <p className="text-xs text-gray-600 dark:text-gray-400">
          Descarga un informe completo con tus archivos subidos, descargas, colecciones y likes.
        </p>

        <div className="flex flex-wrap gap-3">
          <button onClick={() => downloadReport('csv')} className="bg-gray-900 dark:bg-gray-700 text-white px-4 py-2 text-xs rounded">
            Informe (CSV)
          </button>
          <button onClick={() => downloadReport('json')} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 text-xs rounded">
            Informe (JSON)
          </button>
        </div>
      </section>
      ...
      <section className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm rounded-xl p-5">
        <h2 className="font-semibold text-lg text-gray-800 dark:text-gray-100">Mis archivos</h2>

        {files.length === 0 ? (
          <p className="text-sm mt-3 text-gray-500 dark:text-gray-400">Aún no has subido archivos.</p>
        ) : (
              ...
              <tbody>
                {files.map(f => (
                  <tr key={f.id} className="border-b border-gray-200 dark:border-gray-700">
                    ...
                    <td className="py-2">
                      <button onClick={() => handleDownload(f)} className="bg-blue-600 text-white text-xs px-3 py-1 rounded">
                        Descargar
                      </button>
                    </td>
                    <td className="py-2">
                      <button onClick={() => handleDeleteFile(f)} className="bg-red-600 text-white text-xs px-3 py-1 rounded">
                        Eliminar
                      </button>
                    </td>
                  </tr>
